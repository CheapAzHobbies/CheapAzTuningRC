...
- **Servo saver, real talk:** the OEM Traxxas servo saver does absorb impacts day-one, but **all aluminum bell cranks eventually "weld" themselves** — the saver's slip surfaces fuse from heat + friction after enough cycles, and the saver stops slipping. **For racing this is actually desired** (zero slop, direct steering response). **For bashing it's in principle bad** — no impact absorption. In practice on this build: the JX/PTK servos we run take constant crash loads without help and survive multi-year service lives (see [`servo_analysis.md`](servo_analysis.md#notes)), so a functional servo saver is **nice-to-have, not load-bearing**. Run the OEM saver if you have it; don't lose sleep when it eventually welds shut.
