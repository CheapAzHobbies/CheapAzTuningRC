...
The E-Revo 1.0 CVDs are longer than the Jato 4x4 needs, so they get **cut in half and rejoined to length** with a center sleeve. Several ways to make that joint — see **[Build options](#build-options--joining-the-two-cvd-halves)** below. **Final method: slip-fit sleeve + cross-pin + Loctite 648** (welding was tried and broke at the weld). The E-Revo axles seat in the **Jato 4x4 EHD hubs**, with **4 Teflon (Traxxas) washers at the wheel end** and the **E-Revo hubs** (the same hubs run on the Revo and the Slash).
...
> 2. **Build the permanent axles to that length — slip-fit sleeve + cross-pin + Loctite 648.** Welding was the first plan but it **kept breaking at the weld** (brittle HAZ on this plated medium-carbon knock-off steel), so the cross-pin is the permanent lock now — no heat, no HAZ, ignores the metallurgy. It's a wear part anyway, so a **fresh set is easy to remake** if one fails. (See [build options](#build-options--joining-the-two-cvd-halves).)
>
> ⚠️ **WIP** — **front** length dialed: **final 90.5 mm, even halves 45.25 mm each** (flat, ignoring the manufacturing nub; remove 46.1 mm / ~23.05 mm per end from the 136.60 stock; caliper re-zeroed). **Pin** the final front pair to that (welding broke at the weld — see build options). **Rear axle not built yet** — tune it next, then pin both.
>
...
| **Front (adjustable prototype)** | **90.94 mm** (≈91 flat, without the manufacturing nub) | ✅ length dialed | Threaded turnbuckle-style center, adjusted to fit on the car. Re-measured with the caliper re-zeroed: **90.94 mm ignoring the manufacturing nub** (the small cast nub on the cup side) — round to **91 mm flat** for the target. **Final front = 90.5 mm**, **even halves: 45.25 mm each** (tip → cut), joined at center in the sleeve + cross-pin. Remove **46.1 mm** total (136.60 − 90.5) → **~23.05 mm off each inner end**. Cut a hair long and dry-fit in the sleeve before pinning. (Earlier 85.5 mm reading was bad — caliper was decalibrated.) **Shorter = front.** |
| **Rear** | TBD | ⏳ not built yet | Build + tune the rear adjustable prototype next, then pin. |
...
> **Status: CROSS-PIN + SLEEVE is the permanent method.** Live candidates first; struck-through = ruled out. **Welding was tried and kept breaking at the weld** (brittle HAZ on this medium-carbon, plated knock-off steel — see the ruled-out row), so the build is now **threaded/adjustable axle to tune the length, then a slip-fit sleeve + cross-pin + Loctite 648** for the permanent set. The pin is a mechanical lock that ignores the metallurgy that's been cracking the welds.

| Option | How | Status |
|---|---|---|
| ⭐ **Slip-fit sleeve + retaining compound (680/648) + cross-pin** | Steel sleeve over the shaft (tight fit, reamed ~5.6 mm; **long overlap = more bond area**), bond with a **high-strength retaining compound** — **Loctite 680** (strongest, ~5000 psi) or **648**, not threadlocker. **Roughen/scuff the shaft + bore, degrease, prime (7649)** for max grip, then **drive a cross-pin (double shear)** as insurance. No heat, no HAZ, no warp. Permanent. | **Chosen — permanent final.** Retaining compound is the "OP Loctite" for shaft-in-sleeve; the pin is cheap insurance against the reversing-torque shear. Sidesteps the weld failure. |
| 🔵 **Threaded adjustable axle (length-tuning prototype)** | Thread both cut ends into an internally-threaded steel standoff so the length **adjusts**. Fit to the car, dial in each end (**front shorter**), then build the permanent pinned axle to that measured length. | **Prototype only** — used to find the length, then superseded by the pinned set |
...
| 🚫 ~~Threaded + set screws + red Loctite (serviceable)~~ | Thread the ends, set screws on filed flats, red Loctite, torch to adjust | **Dropped for the final part** — serviceability traded away; the pinned sleeve is stronger and remaking a set is easy. (Still the route if you ever want a *serviceable* axle.) |
...
- **Join method (custom axles) — settled:** **tune the length on an adjustable threaded axle, then build the permanent set with a slip-fit sleeve + high-strength retaining compound (Loctite 680/648) + a cross-pin for insurance.** **Welding was tried and kept breaking at the weld** (brittle HAZ on this plated medium-carbon steel), so the sleeve-bond-and-pin is the method. **Shorter axle = front.** It's a wear part, so a fresh set is easy to remake.
