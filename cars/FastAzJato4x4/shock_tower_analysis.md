...
> **Chosen (decision changed): MonsterKingz (G-Maxx) carbon fiber tower set, front + rear** (~$33.29/set). Going CF mainly because it **accommodates the larger D8 big-bore shocks better** with the **HPI Vorza Flux shock standoffs (67410)**, the same **HB HBS67410** standoffs already in hand (HPI/HB cross-brand, see [`shock_analysis.md`](shock_analysis.md#shock-standoffs--mounting)). **3D-printed shock covers** protect the shock caps the CF tower doesn't shield on an upside-down landing. The rear CF tower still carries the OEM TRA9046 wing mount (MonsterKingz rear CF is one of the two towers that fit it, see [`aero_analysis.md`](aero_analysis.md#shock-tower-compatibility-cascade)).
>
> ⚠️ **Accepted trade-off:** this reverses the earlier CF veto. My own experience is that a brittle CF **front** tower can grenade the Jato front end in a direct tower hit (the Jato's tall front tower has more leverage than a Slash's). Going CF anyway with eyes open; the stock #9033 front + TRA9039 rear (below) is the fallback if the CF proves too fragile.
