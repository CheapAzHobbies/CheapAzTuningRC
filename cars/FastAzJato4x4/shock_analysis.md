...
> **Most likely config: HPI Apache C1 (plastic) front + rear** — lighter than the metal D8, and the plastic bodies are protected on this build by the corrected shock-tower geometry (the tower config from last time — see [shock tower analysis](shock_tower_analysis.md)) plus the [arm guards](arm_analysis.md#shock-guards). It's the same 97mm, **16mm big-bore** shock as the metal **Hot Bodies D8 (HBS67296)**, which stays the fallback only if shock bodies still get destroyed.
>
