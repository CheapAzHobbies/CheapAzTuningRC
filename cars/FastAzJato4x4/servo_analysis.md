...
- **JX failure mode (why PTK is the successor):** the JX CLS6322HVs don't blow up — they **lose the ability to center reliably**. **Two have had to be retired** with two distinct symptoms of the same underlying drift in the pot / centering circuit:
  1. **Dies holding an offset** — servo locks somewhere off-neutral and won't return. Car pulls one way at rest, eats one tire. *(Oldest unit in the bin retired this way yesterday — 2026-05-31.)*
  ...
  Both modes are track-killing well before the gears or motor give up, and both are the empirical reason the PTK 9752TG-D is the going-forward pick rather than reordering JX.
