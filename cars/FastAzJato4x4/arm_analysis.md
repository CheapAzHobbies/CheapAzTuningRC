...
## Notes
- **Track width, not wheelbase.** Longer suspension arms move the wheel *outboard*, which is track width. Wheelbase is front axle to rear axle and no arm changes it — that takes moving the bulkheads. The giveaway is that the FLM gain is quoted **per side**, which is a track measurement, and the clincher is the [custom axles](driveshaft_analysis.md#shortening--joining-e-revo-cvds-custom-axles-wip): driveshaft length is set by diff outdrive to hub, a lateral distance, so a genuine wheelbase change would need no new axles at all.
