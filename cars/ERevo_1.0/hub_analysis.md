...
- **Decision (revised): Enron metal front + RPM True Track rear.** The RPM 80582 plastic front blocks failed in the field — even new, the **pillow balls pull out every other crash** — so the front moves to **Enron metal** (won't crack, holds the balls). **Reuse the same oversized 6×15×5 / 12×21×5 bearings** (already in hand) in the Enron carriers. Rear stays RPM True Track (4 mm pin lower mount, no pull-out). Enron trade-offs accepted: heavier, and **no part number to reorder (sourcing TBD)**. The prior plan was RPM plastic both ends; that's retired up front. The stock **TRA5334** and Traxxas **5334R** remain reference only.
- **Open item — source the Enron carriers.** No part number, so they need tracking down (Enron/AliExpress "Revo/E-Revo aluminum axle carrier"). Also confirm the Enron uses its own pillow-ball capture hardware, since the **RPM 80010** setscrews/caps are unconfirmed on the Enron.
