...
- [Price History](#price-history) — what the shocks actually cost
...
| **Lightweight body** | May | The shocks sit **inboard** (rocker-actuated, behind the mounts), so they take no crash hits, so no need for a heavy metal body; lighter plastic is the win |
...
- **4,000 cSt fixed the digging, confirmed on track.** At 2,000 cSt the truck packed down far enough on a jump landing that the chassis caught the dirt and the skid plate took an occasional smack. At 4,000 it holds up through the landing, **the intermittent skid-plate hits are gone, and the skid plate lasts noticeably longer** as a result (see [`skid_plate_analysis.md`](skid_plate_analysis.md)). Damping was the fix, not a thicker skid plate. If it ever starts **kicking back** instead of absorbing, that's the too-stiff failure and **3,000 cSt** is the step down, or blend to ~2,830 with equal parts 2,000 and 4,000.
...
Past ~1325 cSt the wt label stops existing and **every brand prints cSt**, so no conversion is needed at all. Silicone diff fluid is the same fluid as shock oil, just sold in the grades shock oil doesn't reach, which is what this truck runs.
