...
>
> - **HOOVO 4200**, lightest, drops into the same tray. **A set from Mike, so it's the one that actually gets tried.** HV labeling contradicts itself
...
|:---|:---:|:---:|:---:|:---:|:---:|:---:|:---|
| **HOOVO** | **214.4g** | **223** | <4mΩ | 120C | **18-30C** | 93 × 43.8 × 25 | $31.50 paid |
...
- **Still to log:** **IR on all four Premos** (quick, and it gives both a health baseline at 93 cycles and a pair-matching check), and a full discharge capacity test on one. The ~3000mAh charged in from 3.8V storage is a good sign but isn't the same measurement, so the true number on a 93-cycle pack is still unknown. Whatever replaces them should get an IR reading on arrival, before the first run, as the baseline to age against.
- **First jobs on the HOOVOs from Mike:** **IR on each pack before the first run**. It fills the only empty column in the table above, gives a true C to check the 120C claim against, and confirms the pair is matched. Then **settle the HV question**: charge one to 4.35V/cell and see whether it takes it and holds, since the listing says 11.4V but also says never above 4.2V. Those two readings turn the HOOVO row from published numbers into measured ones.
