...
- **Usage-based lifespan, not just calendar time: the rear metal plate wore out over ~39 battery-pair sessions.** Traced from the cycle counts in [`batteries/README.md`](../../batteries/README.md): the Zeee Premo pair logged +36 cycles (70 → 106) between 2026-05-27, when the tracker started, and now, plus +3 from the HOOVO pair (acquired 8/31/26). The tracker started 17 days after the skid plate did (5/10 → 5/27), so the real total is 39 or a little more. Runs-per-plate is the more honest wear metric than days for a part like this, log it the same way for the plastic replacement.
